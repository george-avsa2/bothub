...
export class UserStore {
  ...
  async updateBalance(id: string, newBalance: number): Promise<User | null> {
    const conn = await client.connect();
    try {
      const sql = 'UPDATE users SET balance = $1 WHERE id = $2 RETURNING *';
      const result = await conn.query(sql, [newBalance, id]);
      ...
      throw new Error(`Cannot update balance. ${err}`);
    } finally {
      conn.release();
    }
  }
